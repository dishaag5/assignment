...
AZURE_EMBED_DEPLOYMENT = "embedding-3-small"  # embeddings
...
df_embed.write.mode("overwrite").saveAsTable("rag_pdf_chunks_embedded")
...
answer_runnable = RunnableLambda(lambda q: generate_answer_with_context(q["query"], q["docs"]))

branch_runnable = RunnableBranch(
    # If it's a math question → math function
    (
    ...
    (
        ...
        retriever_runnable |
        (lambda docs, prompt=None: {"query": prompt, "docs": docs}) |
        answer_runnable
    )
)
...
response = branch_runnable.invoke(query)
print(response)
